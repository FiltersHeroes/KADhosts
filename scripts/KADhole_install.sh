...
# Script based on https://github.com/mmotti/pihole-regex/blob/master/install.sh

# Set variables
db_gravity='/etc/pihole/gravity.db'
file_pihole_regex='/etc/pihole/regex.list'
file_KAD_regex='/etc/pihole/KADhole.list'
file_KAD_remote_regex='https://raw.githubusercontent.com/PolishFiltersTeam/KADhosts/master/KADhole.txt'
installer_comment='github.com/PolishFiltersTeam/KADhosts'
...
echo "[i] Fetching KADhole's regexps"
# Fetch the remote regex file and remove comment lines
KAD_remote_regex=$(wget -qO - "${file_KAD_remote_regex}" | grep '^[^#]')
# Conditional exit if empty
[[ -z "${KAD_remote_regex}" ]] && { echo '[i] Failed to download KAD regex list'; exit 1; }
...
if [[ "${usingDB}" == true ]]; then
	...
	if [[ -n "${str_regex}" ]] ; then
		...
		if [[ -n "${db_migrated_regexps}" ]]; then
			...
			updatesRequired=$(comm -3 <(sort <<< "${db_migrated_regexps}") <(sort <<< "${KAD_remote_regex}"))
			...
			# If we have a local KAD-regex.list, read from that as it was used on the last install (pre-db)
			# Otherwise, default to the latest remote copy
			if [[ -s "${file_KAD_regex}" ]]; then
				# Only return regexps in both the regex table and regex file
				mapfile -t result <<< "$(comm -12 <(sort <<< "${str_regex}") <(sort < "${file_KAD_regex}"))"

			else
				# Only return regexps in both the regex table and regex file
				mapfile -t result <<< "$(comm -12 <(sort <<< "${str_regex}") <(sort <<< "${KAD_remote_regex}"))"
			fi
	...
	csv_file=$(generateCSV "${KAD_remote_regex}")
	...
	# Remove the old KAD-regex file
	[[ -e "${file_KAD_regex}" ]] && sudo rm -f "${file_KAD_regex}"
	...
	if [[ -n "${str_regex}" ]]; then
		...
		if [[ -s "${file_KAD_regex}" ]]; then
			echo "[i] Removing KADhole.list from a previous install"
			comm -23 <(sort <<< "${str_regex}") <(sort "${file_KAD_regex}") | sudo tee $file_pihole_regex > /dev/null
			sudo rm -f "${file_KAD_regex}"
		else
			# In the event that file_KAD_regex is not available
			# Match against the latest remote list instead
			echo "[i] Removing KADhole.list from a previous install"
			comm -23 <(sort <<< "${str_regex}") <(sort <<< "${KAD_remote_regex}") | sudo tee $file_pihole_regex > /dev/null
		fi
	fi

	# Copy latest regex list to file_KAD_regex dir
	echo "[i] Copying remote KADhole.list to ${file_KAD_regex}"
	echo "${KAD_remote_regex}" | sudo tee "${file_KAD_regex}" > /dev/null

	# Status update
	echo "[i] $(wc -l <<< "${KAD_remote_regex}") regexps found in KADhole.list"

	# If pihole regex is not empty after changes
	if [[ -s "${file_pihole_regex}" ]]; then
		# Extract non KAD-regex entries
		existing_regex_list="$(grep '^[^#]' < "${file_pihole_regex}")"
		# Form output (preserving existing config)
		echo "[i] $(wc -l <<< "$existing_regex_list") regexps exist outside of KADhole.list"
		final_regex=$(printf "%s\n" "${KAD_remote_regex}" "${existing_regex_list}")
	else
		echo "[i] No regex.list differences to KADhole.list"
		final_regex=$(printf "%s\n" "$KAD_remote_regex")
	fi
